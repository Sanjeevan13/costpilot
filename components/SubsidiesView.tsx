...
const SubsidiesView: React.FC<{ userProfile: UserProfile }> = ({ userProfile }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Government Relief & Subsidies</h1>
        <p className="text-slate-500">Matched programs based on your profile: Income RM {userProfile.income}, Household {userProfile.householdSize}.</p>
      </div>
        ...
        {subsidies.map((subsidy, idx) => (
          <div key={idx} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="p-5 border-b border-slate-100 flex justify-between items-start">
               <div>
                 <h3 className="font-bold text-slate-900">{subsidy.name}</h3>
                 <span className="text-xs text-slate-500">{subsidy.amount}</span>
               </div>
               {subsidy.status === 'Eligible' && <CheckCircle className="text-emerald-500" size={20} />}
               {subsidy.status === 'Not Eligible' && <XCircle className="text-slate-300" size={20} />}
               {subsidy.status === 'Recommended' && <AlertCircle className="text-blue-500" size={20} />}
            </div>
            <div className="p-5 flex-grow">
              <p className="text-sm text-slate-600 mb-4">{subsidy.description}</p>
              
              {subsidy.reason && (
                <div className="bg-slate-50 p-2 rounded text-xs text-slate-500 mb-3">
                ...
                <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full ${
                      subsidy.probability > 80 ? 'bg-emerald-500' : 
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100 mt-auto">
               <button className="w-full bg-white border border-slate-300 text-slate-700 py-2 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors flex items-center justify-center gap-2">
