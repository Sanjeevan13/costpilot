...
const DashboardView: React.FC<{ userProfile: UserProfile; setView: (view: ViewState) => void }> = ({ userProfile, setView }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Pass full profile for subsidies match in unified endpoint
  ...
  if (!stressData && !signals) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4 animate-fade-in text-center p-4">
        <AlertCircle className="text-red-500 h-16 w-16 mb-2" />
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">Unable to load financial data</h2>
        <p className="text-slate-500 dark:text-slate-400 max-w-md">
          We couldn't connect to the analysis engine. Please check your internet connection and try again.
        </p>
        <button
  ...
  const rentBurden = userProfile.income > 0
    ? Math.round((userProfile.rent / userProfile.income) * 100)
    : 0;

  const monthlyBalance = signals?.numbers?.monthlyBalance ?? 0;
  const overspending = signals?.overspending || {};
  const attentionItems = Object.entries(overspending as Record<string, boolean>)
    .filter(([_, isHigh]) => isHigh)
    .map(([key]) => key);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 animate-fade-in">
      <div className="mb-8">
        ...
        {attentionItems.length > 0 && (
          <div className="mt-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4 flex items-start gap-3">
            ...
            <div>
              <h3 className="font-bold text-orange-900 dark:text-orange-200 text-sm">Attention Needed</h3>
              <p className="text-sm text-orange-800 dark:text-orange-300">
                You are overspending on: <span className="font-bold">{attentionItems.join(', ')}</span>.
                Consider reviewing your budget in these areas.
              </p>
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{subsidyCount} <span className="text-sm font-normal text-slate-400 dark:text-slate-500">programs</span></h3>
        </div>

        {/* Inflation Pressure (Was Buying Power) */}
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="flex justify-between items-start mb-4">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Inflation Pressure</p>
          <h3 className="text-2xl font-bold text-slate-900 dark:text-white">3.8% <span className="text-sm font-normal text-slate-400 dark:text-slate-500">est. YoY</span></h3>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm mb-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="h-4 w-full bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full ${monthlyBalance < 0
                  ? 'bg-red-500'
                  : 'bg-blue-500'
                  }`}
                style={{ width: `${userProfile.income > 0 ? Math.min(((userProfile.rent + (userProfile.utilities || 0) + (userProfile.transportCost || 0) + (userProfile.food || 0) + (userProfile.debt || 0) + (userProfile.subscriptions || 0)) / userProfile.income) * 100, 100) : 0}%` }}
              ></div>
            </div>
          </div>
        </div>
