...
const INPUT_CLASS = "w-full bg-slate-50 dark:bg-[#1a1238] border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-slate-900 dark:text-white text-sm focus:outline-none focus:border-[#b55cff]/50 dark:focus:border-[#a07cf6]/50 focus:ring-1 focus:ring-[#b55cff]/30 dark:focus:ring-[#a07cf6]/30 placeholder-slate-400 dark:placeholder-slate-500 transition-all";
const LABEL_CLASS = "text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 mb-1.5 block";
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white">Your Travel Routes</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Add your frequent routes and let AI find cheaper alternatives</p>
                </div>
                <div className="flex gap-3">
                    <button onClick={() => setShowForm(!showForm)}
                        className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#b55cff] text-white dark:text-[#120b22] text-sm font-bold uppercase tracking-wider hover:bg-[#c980ff] transition-all shadow-md dark:shadow-none">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 animate-fade-in">
                <div className="bg-white dark:bg-[#120b22] rounded-2xl border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-none p-5 flex flex-col justify-between transition-colors">
                    <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 mb-2">Monthly Income</p>
                    <p className="text-2xl font-black text-slate-900 dark:text-white">RM{(userProfile.income || 0).toFixed(2)}</p>
                </div>
                <div className="bg-white dark:bg-[#120b22] rounded-2xl border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-none p-5 flex flex-col justify-between transition-colors">
                    <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 mb-2">
                    ...
                    <div className="flex items-baseline gap-2">
                        <p className={`text-2xl font-black ${((userProfile.appliedLifestyleOptimizations || []).filter(o => o.category === 'transport' || o.category === 'transportation').reduce((sum, o) => sum + o.monthlySavings, 0)) > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-900 dark:text-white'}`}>
                        ...
                        {((userProfile.appliedLifestyleOptimizations || []).filter(o => o.category === 'transport' || o.category === 'transportation').reduce((sum, o) => sum + o.monthlySavings, 0)) > 0 && (
                            <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-500/60 uppercase">Optimized</span>
                        )}
                    </div>
                </div>
                <div className="bg-white dark:bg-[#120b22] rounded-2xl border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-none p-5 flex flex-col justify-between transition-colors">
                    <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#b55cff] dark:text-[#a07cf6] mb-2">Total Transport Savings</p>
            ...
            {showForm && (
                <div className="bg-white dark:bg-[#120b22] rounded-2xl border border-slate-200 dark:border-[#a07cf6]/20 shadow-lg dark:shadow-none p-6 space-y-5 animate-fade-in transition-colors">
                    <div className="flex items-center gap-2 mb-2">
                        <Route className="text-[#b55cff] dark:text-[#a07cf6]" size={18} />
                        <h4 className="text-sm font-bold uppercase tracking-widest text-[#b55cff] dark:text-[#d48aff]">New Travel Route</h4>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="flex items-end pb-1">
                            <label className="flex items-center gap-3 cursor-pointer group">
                                <div className={`w-12 h-7 rounded-full border-2 relative transition-all duration-300 ${form.isRoundTrip ? 'bg-[#b55cff]/10 dark:bg-[#b55cff]/30 border-[#b55cff]/50' : 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-white/10'}`}
                                    onClick={() => setForm({ ...form, isRoundTrip: !form.isRoundTrip })}>
                                    <div className={`w-5 h-5 rounded-full absolute top-0.5 transition-all duration-300 ${form.isRoundTrip ? 'left-[22px] bg-[#b55cff]' : 'left-0.5 bg-slate-400 dark:bg-slate-500'}`} />
                                </div>
                                <span className="text-sm text-slate-500 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">Round Trip</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        <button onClick={() => setShowForm(false)} className="px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">Cancel</button>
                        <button onClick={handleAddRoute} disabled={saving || !form.startLocation || !form.destination}
                            className="px-8 py-3 rounded-xl bg-[#b55cff] text-white dark:text-[#120b22] text-xs font-bold uppercase tracking-wider hover:bg-[#c980ff] transition-all shadow-md dark:shadow-none disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2">
            ...
            {/* Route Cards */}
            {
                routes.length === 0 && !showForm ? (
                    <div className="py-20 text-center flex flex-col items-center justify-center border border-slate-200 dark:border-white/5 rounded-2xl bg-white dark:bg-[#120c22]/50 shadow-sm dark:shadow-none transition-colors">
                        <div className="w-16 h-16 rounded-full bg-slate-50 dark:bg-white/5 flex items-center justify-center mb-4 border border-slate-200 dark:border-white/10">
                            <Car className="text-slate-400 dark:text-slate-500" size={24} />
                        </div>
                        <h3 className="text-xl font-bold mb-2 text-slate-900 dark:text-white">No Routes Added</h3>
                    ...
                    <div className="space-y-4">
                        ...
                        {routes.map(route => {
                            ...
                            return (
                                <div key={route.id} className="bg-white dark:bg-[#120b22] rounded-2xl shadow-sm dark:shadow-none border border-slate-200 dark:border-white/5 overflow-hidden hover:border-[#a07cf6]/40 dark:hover:border-[#a07cf6]/20 hover:shadow-md dark:hover:shadow-none transition-all">
                                    {/* Route Header */}
                                    <div className="p-5 flex items-center justify-between">
                                        <div className="flex items-center gap-4 flex-1 min-w-0">
                                            <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-[#1a1238] border border-slate-200 dark:border-[#a07cf6]/20 flex items-center justify-center shrink-0">
                                                <Car className="text-[#b55cff] dark:text-[#a07cf6]" size={16} />
                                            </div>
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                            {!route.aiScenarios && (
                                                <button onClick={() => handleAnalyzeRoute(route)} disabled={isAnalyzing}
                                                    className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[#b55cff]/10 dark:bg-[#a07cf6]/10 border border-[#b55cff]/20 dark:border-[#a07cf6]/20 text-[#b55cff] dark:text-[#d48aff] text-sm font-bold hover:bg-[#b55cff]/20 dark:hover:bg-[#a07cf6]/20 transition-all disabled:opacity-50">
                                            ...
                                            {route.aiScenarios && (
                                                <button onClick={() => setExpandedRoute(isExpanded ? null : route.id)}
                                                    className="flex items-center gap-1 px-3 py-2 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-bold hover:bg-emerald-100 dark:hover:bg-emerald-500/20 transition-all">
                                                    {isExpanded ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
                                                    {isExpanded ? 'Hide' : 'View'} AI Scenarios
                                                </button>
                                            )}
                                            <button onClick={() => handleDeleteRoute(route.id)}
                                                className="p-2 rounded-lg text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-400/10 transition-all">
                                                <Trash2 size={14} />
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    {isExpanded && route.aiScenarios && (
                                        <div className="border-t border-slate-200 dark:border-white/5 p-6 space-y-5 animate-fade-in bg-slate-50 dark:bg-[#0d0820]">
                                            <div className="flex items-center justify-between">
                                                <h4 className="text-sm font-bold uppercase tracking-[0.2em] text-[#b55cff] dark:text-[#a07cf6]">AI-Optimized Scenarios</h4>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                {(['cheapest_option', 'balanced_option', 'fastest_option'] as const).map((key) => {
                                                    ...
                                                    const labels: Record<string, { label: string; color: string }> = {
                                                        cheapest_option: { label: '💰 Cheapest', color: 'text-emerald-600 dark:text-emerald-400' },
                                                        balanced_option: { label: '⚖️ Balanced', color: 'text-blue-600 dark:text-blue-400' },
                                                        fastest_option: { label: '⚡ Fastest', color: 'text-amber-600 dark:text-amber-400' },
                                                    };
                                                    ...
                                                    return (
                                                        <div key={key}
                                                            ...
                                                            className={`rounded-xl border p-5 cursor-pointer transition-all duration-300 ${isSelected
                                                                ? 'bg-white dark:bg-[#1a1238] border-[#b55cff] dark:border-[#a07cf6]/40 shadow-md dark:shadow-[0_0_20px_rgba(160,124,246,0.1)]'
                                                                : 'bg-white dark:bg-[#120b22] border-slate-200 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/10 shadow-sm dark:shadow-none'}`}>
                                                            ...
                                                            <p className="text-slate-900 dark:text-white font-bold text-base mb-2">{scenario.method}</p>
                                                            <p className="text-slate-600 dark:text-slate-400 text-sm mb-4 leading-relaxed">{scenario.reasoning}</p>
                                                            <div className="space-y-2.5">
                                                                <div className="flex justify-between text-sm">
                                                                    <span className="text-slate-500">Per trip</span>
                                                                    <span className="text-slate-900 dark:text-white font-bold">RM{scenario.estimated_cost_per_trip?.toFixed(2)}</span>
                                                                </div>
                                                                <div className="flex justify-between text-sm">
                                                                    <span className="text-slate-500">Monthly</span>
                                                                    <span className="text-slate-900 dark:text-white font-bold">RM{scenario.monthly_cost?.toFixed(0)}</span>
                                                                </div>
                                                                <div className="flex justify-between text-sm">
                                                                    <span className="text-slate-500">Time</span>
                                                                    <span className="text-slate-700 dark:text-slate-300">{scenario.estimated_time}</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    );
                                            ...
                                            <div className="flex flex-col sm:flex-row items-center justify-between bg-white dark:bg-[#1a1238]/50 rounded-xl p-6 border border-slate-200 dark:border-[#a07cf6]/20 gap-6 shadow-sm dark:shadow-[0_4px_20px_rgba(0,0,0,0.2)]">
                                                <div className="flex items-center gap-10 w-full sm:w-auto">
                                                    <div className="flex flex-col">
                                                        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-500 mb-2">Original Cost</p>
                                                        <p className="text-2xl font-black text-slate-400 dark:text-white/40 line-through decoration-red-400/50 dark:decoration-red-500/50">RM{route.monthlyCost.toFixed(0)}</p>
                                                    </div>
                                                    <div className="flex items-center justify-center w-10 h-10 rounded-full bg-[#b55cff]/10 dark:bg-[#a07cf6]/10">
                                                        <ArrowRight className="text-[#b55cff] dark:text-[#a07cf6]" size={20} />
                                                    </div>
                                                    <div className="flex flex-col">
                                                        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400 mb-2">Monthly Saving</p>
                                                        <div className="flex items-baseline gap-1">
                                                            ...
                                                            <span className="text-emerald-500 dark:text-emerald-500/60 text-xs font-bold uppercase tracking-tighter">/ mo</span>
                                                        </div>
                                                    </div>
                                                </div>

                                                <div className="w-full sm:w-auto border-t sm:border-t-0 sm:border-l border-slate-200 dark:border-white/5 pt-6 sm:pt-0 sm:pl-8">
                                                    {userProfile.appliedLifestyleOptimizations?.some(o => o.title === `Transport: ${route.id}`) ? (
                                                        <button
                                                            onClick={() => handleUnapplyOptimization(route.id)}
                                                            className="w-full sm:w-auto px-10 py-4 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 border-2 border-emerald-200 dark:border-emerald-500/30 text-emerald-600 dark:text-emerald-400 text-base font-black flex items-center justify-center gap-3 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 transition-all shadow-sm dark:shadow-[0_0_15px_rgba(16,185,129,0.1)]"
                                                        >
